...
└── nginx/                   # 挂载到容器 /usr/local/openresty/nginx
    ...
    └── lua/                 # Lua 脚本目录
        ...
        ├── or-gallery.html # Gallery SPA（图片/漫画画廊 + 内置阅读器，访问 /or-gallery）
        ├── resty/           # 自定义 resty 库
        │   └── ctxvar.lua   # 请求上下文变量封装
        └── lib/             # 内置 Lua 库
            ├── basic_auth.lua   # HTTP 基础认证
            ├── dirapi.lua       # 目录 JSON API（/api/ls/）
            ├── fileapi.lua      # 文件管理 API（/api/rm、move、mkdir、upload）
            ├── preview_inject.lua # HTML 目录页媒体预览注入
            ├── tap.lua          # 流量 Tap / 调试工具
            ├── vips.lua         # 动态图片处理（libvips + lua-vips）
            ├── webdav.lua       # WebDAV 处理器
            ├── wol.lua          # Wake-on-LAN（网络唤醒）
            └── zipfs.lua        # ZIP 虚拟文件系统
```
...
| **Gallery 画廊浏览器（新）** | 专为图片/漫画收藏设计的 SPA 画廊：封面卡片浏览（含 vips 缩略图）、内置漫画阅读器（单/双页、LTR/RTL、触屏翻页、全屏）、支持目录和 ZIP/CBZ 直接阅读 | 访问 `/or-gallery?path=/your/dir` 或 `/or-gallery?path=/your/dir&page=1&reader=/path/to/gallery.zip` |
